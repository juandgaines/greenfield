...
Number each finding sequentially. Skip the `## Tests` and `## Notes` sections — those are framework context.

If the report's verdict is `SKIPPED` or `INVALID_INVOCATION`, write a single audit row noting the short-circuit and stop. There are no per-finding rows to emit.
...
- The report has no `## Findings` section AND verdict is not `SKIPPED` / `INVALID_INVOCATION`. Write the table header and a single row noting "no findings to audit."
